...
| ポップアップ/カード: 回答提案 | 質問検出時に「この質問への回答例」を提示、コピーボタン | answer_suggestionイベントで表示 |
...
async def on_final_segment(self, segment: TranscriptEvent):
    ...
    asyncio.create_task(self._maybe_answer_suggestion(segment)) # 優先度3
...
**質問検出 → 回答提案(優先度3)**
- 話者への質問かどうかを分類(疑問形・語尾・文脈から判定)。誤検知を減らすため、直前数ターンの文脈込みでLLM判定。
- 質問と判定されたら、直近の会話コンテキストを渡して回答案を生成。低レイテンシ重視のため軽量モデル(例: Claude Haiku相当)を優先し、必要なら詳細版を後追いで再生成する2段階方式も検討可。
